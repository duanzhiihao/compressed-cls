...
def _load_model(
    ...
    if pretrained:
        ...
        root_dir = MYCV_DIR / 'weights/compressai'
        if not root_dir.is_dir():
            print(f'Making dir {root_dir}...')
            root_dir.mkdir(parents=False, exist_ok=False)
        wname = os.path.basename(url)
        if wname in os.listdir(root_dir):
            wpath = root_dir / wname
            if progress:
                print(f'Loading pre-trained parameters from {wpath}...')
            state_dict = torch.load(wpath)
        else:
            state_dict = load_state_dict_from_url(url, model_dir=root_dir, progress=progress)
